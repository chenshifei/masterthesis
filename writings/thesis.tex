...
\begin{table}
  ...
  \begin{tabular}{*{6}{|l}|}
    \hline
    Language & BLEU & P@1 & P@2 & P@3 & P@4 \\ [0.25ex]
    \hline\hline
...
The results are in Table \ref{table:altering_lang_id}. The source tag did not change the overall result much, which is in line with previous claims and results \parencite{Johnson:2016aa, Blackwood:2018aa}. The difference between different langauge annotations indicated that a word embedding based MNMT system would also automatically learn the source language during training. People should only annotate the target language into the source text.

\subsection{Analysis on the Effect of Language Similarity}
